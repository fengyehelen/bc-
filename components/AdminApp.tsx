...
const AdminApp: React.FC<AdminAppProps> = (props) => {
  ...
  const [taskCountry, setTaskCountry] = useState<string>('id');
  ...
  const getCountryFromCurrency = (currency: string): string => {
      const entry = Object.entries(LANGUAGES).find(([k, v]) => v.currency === currency);
      return entry ? entry[1].label : 'Unknown';
  };
  ...
  const handleSendMessage = () => {
    ...
    props.sendMessage(msgData.userId, msgData.title, msgData.content, amount);
           ...
           <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
             ...
             <table className="w-full text-left">
                <thead className="bg-slate-50 border-b border-slate-200">
                    <tr><th className="p-4">User</th><th className="p-4">Balance</th><th className="p-4">Total Earned</th><th className="p-4">Country</th><th className="p-4">Actions</th></tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                   {sortedUsers.map(u => (
                      <tr key={u.id} className={`hover:bg-slate-50 ${u.isBanned ? 'bg-red-50' : ''}`}>
                         ...
                         <td className="p-4 text-xs font-bold uppercase text-slate-500">{getCountryFromCurrency(u.currency)}</td>
                         <td className="p-4 flex gap-2">
                             ...
                             <button onClick={() => props.manageContent('user', u.id, 'ban')} className={`text-xs border px-2 py-1 rounded flex items-center gap-1 ${u.isBanned ? 'border-green-300 bg-green-50 text-green-600' : 'border-red-300 bg-red-50 text-red-600'}`}>
               ...
               <div className="bg-white p-6 rounded-xl shadow-lg border border-indigo-100">
                  ...
                  <div className="grid grid-cols-2 gap-6">
                     ...
                     <div className="space-y-4">
                         ...
                         <div className="space-y-2"><label className="text-xs font-bold uppercase text-slate-500">Target Country</label><select value={taskCountry} onChange={e => setTaskCountry(e.target.value)} className="w-full border border-slate-300 p-2.5 rounded-lg focus:outline-none bg-white">{COUNTRIES.map(c => (<option key={c.value} value={c.value}>{c.label}</option>))}</select></div>
                     </div>
                  </div>
                  <div className="mt-6 pt-4 border-t border-slate-100 flex justify-end"><button onClick={publishTask} className="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-3 rounded-xl font-bold shadow-lg shadow-indigo-200 flex items-center gap-2 transition-transform hover:-translate-y-1 active:scale-95"><Save size={18} /> Publish Task</button></div>
               </div>
               ...
               <div className="bg-white p-6 rounded-xl shadow-lg border border-indigo-100">
                  ...
                  <div className="grid grid-cols-2 gap-6">
                       ...
                       <div className="space-y-4">
                           ...
                           <div><label className="text-xs font-bold uppercase text-slate-500 mb-2 block">Target Country</label><select value={actCountry} onChange={e => setActCountry(e.target.value)} className="w-full border border-slate-300 p-2.5 rounded-lg focus:outline-none bg-white">{COUNTRIES.map(c => (<option key={c.value} value={c.value}>{c.label}</option>))}</select></div>
                       </div>
                  </div>
           ...
           <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 max-w-2xl relative">
              ...
              <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-8">
                      <div><label className="block text-xs font-bold text-slate-500 uppercase mb-2">New User Bonus</label>{COUNTRIES.map(c => (<div key={c.value} className="flex items-center mb-1"><span className="w-10 text-[10px] font-bold uppercase">{c.value}</span><input type="number" value={props.config.initialBalance[c.value] || 0} onChange={e => handleConfigChange('init', c.value, e.target.value)} className="flex-1 border p-1 rounded text-sm" /></div>))}</div>
                      <div><label className="block text-xs font-bold text-slate-500 uppercase mb-2">Min Withdrawal</label>{COUNTRIES.map(c => (<div key={c.value} className="flex items-center mb-1"><span className="w-10 text-[10px] font-bold uppercase">{c.value}</span><input type="number" value={props.config.minWithdrawAmount[c.value] || 0} onChange={e => handleConfigChange('min', c.value, e.target.value)} className="flex-1 border p-1 rounded text-sm" /></div>))}</div>
                  </div>
                  <div><h4 className="text-sm font-bold mb-2">Telegram Channels</h4><div className="grid gap-2">{COUNTRIES.map(c => (<div key={c.value} className="flex items-center gap-2"><span className="w-16 text-xs font-bold uppercase">{c.label}</span><input type="text" value={props.config.telegramLinks[c.value] || ''} onChange={e => props.updateConfig({...props.config, telegramLinks: {...props.config.telegramLinks, [c.value]: e.target.value}})} className="flex-1 border p-2 rounded" placeholder={`https://t.me/betbounty_${c.value}`} /></div>))}</div></div>
                  <div><h4 className="text-sm font-bold mb-2">Static Pages</h4><div className="space-y-4"><div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Help Center Content</label><textarea value={props.config.helpContent || ''} onChange={e => props.updateConfig({...props.config, helpContent: e.target.value})} className="w-full border p-2 rounded h-24" /></div><div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">About Us Content</label><textarea value={props.config.aboutContent || ''} onChange={e => props.updateConfig({...props.config, aboutContent: e.target.value})} className="w-full border p-2 rounded h-24" /></div></div></div>
              </div>
           </div>
        )}
