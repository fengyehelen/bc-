...
import { 
  ...
  Share2, Facebook, Twitter, Link as LinkIcon, Send, MessageCircle
} from 'lucide-react';
...
export const UserLogin: React.FC<{ onAuth: (p: string, pw: string, isReg: boolean, invite?: string) => string | null; t: any; lang: Language }> = ({ onAuth, t, lang }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-6 relative">
       <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-yellow-400 to-amber-600 flex items-center justify-center shadow-xl shadow-yellow-500/20 mb-6"><Sparkles size={40} className="text-white" /></div>
      ...
      <div className="w-full max-w-xs space-y-4">
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 flex items-center space-x-3">
            ...
            <input type={showPwd ? "text" : "password"} value={password} onChange={e=>setPassword(e.target.value)} placeholder={t.password} className="bg-transparent text-white w-full focus:outline-none" />
            <button onClick={() => setShowPwd(!showPwd)} className="text-slate-500 hover:text-white">{showPwd ? <EyeOff size={18} /> : <Eye size={18} />}</button>
        </div>
        {isRegister && (
            <>
        ...
        <button onClick={handleAuth} className="w-full bg-gradient-to-r from-yellow-500 to-amber-600 text-slate-900 font-bold py-3.5 rounded-xl shadow-lg mt-4">{isRegister ? t.register : t.login}</button>
        <div className="flex justify-center text-xs text-slate-400 mt-4 px-1"><button onClick={() => setIsRegister(!isRegister)} className="hover:text-white">{isRegister ? t.login : t.register}</button></div>
        <div className="mt-12 pt-6 border-t border-slate-800 flex justify-center"><a href="#/admin" className="text-slate-600 text-xs flex items-center gap-2 hover:text-yellow-500"><ShieldCheck size={14} /><span>{t.merchantLogin}</span></a></div>
      </div>
    </div>
  );
};
